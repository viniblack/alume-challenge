...
export function DataTable<TData, TValue>({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Paginação igual a sua atual */}
      <div className="flex items-center justify-end space-x-2">
        <span className="hidden sm:flex">Página {pageIndex} de {totalPages}</span>
        <Button onClick={() => onPageChange(pageIndex - 1)} disabled={pageIndex <= 1}>
          Anterior
        </Button>
        <Button onClick={() => onPageChange(pageIndex + 1)} disabled={pageIndex >= totalPages}>
          Próxima
        </Button>
      </div>
    </div>
